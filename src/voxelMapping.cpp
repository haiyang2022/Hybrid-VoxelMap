...
int NUM_FEAT = 0;
int NUM_NDT = 0;
uint32_t unique_id_seg = 0;
uint32_t last_unique_id = -1;
pcl::PointCloud<pcl::PointXYZI>::Ptr world_lidar_seg(new pcl::PointCloud<pcl::PointXYZI>);
sensor_msgs::PointCloud2::ConstPtr latestSegMsg = nullptr;
...
void publish_surf_frame_world(const ros::Publisher &pubLaserCloud, const int point_skip)
{
...
void publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
    ...
    // Save as TUM format
    const std::string filename = "/Downloads/HybridVM_result/result.txt";
...
void publishLidar2seg(const ros::Publisher &pub,
                      const pcl::PointCloud<pcl::PointXYZI>::Ptr &world_lidar)
{
    if (world_lidar->points.size() < 100)
    {
        ROS_WARN("world_lidar only has %lu points (<100), skipping publish", world_lidar->points.size());
    }
    ROS_INFO("world_lidar has %lu points", world_lidar->points.size());
    sensor_msgs::PointCloud2 cloud_msg;
    pcl::toROSMsg(*world_lidar, cloud_msg);
    cloud_msg.header.stamp = ros::Time::now();
    cloud_msg.header.frame_id = "camera_init";
    cloud_msg.header.seq = unique_id_seg;

    pub.publish(cloud_msg);
    ROS_INFO("Published lidar point cloud with unique id: %u", unique_id_seg);
}

bool processSegmentation(
    std::vector<pointWithCov> &pv_list_sem)
{
    ros::spinOnce();
    if (latestSegMsg && (latestSegMsg->header.seq != last_unique_id))
    {
        ROS_INFO("Segmentation result from unique id: %zu", latestSegMsg->header.seq);
        last_unique_id = latestSegMsg->header.seq;
        pcl::PointCloud<pcl::PointXYZRGB> cloud_color;
        pcl::fromROSMsg(*latestSegMsg, cloud_color);

        for (size_t i = 0; i < cloud_color.points.size(); ++i)
        {
            pcl::PointXYZRGB pt = cloud_color.points[i];
            ...
            {
                ROS_WARN("!!!Unknown color: [%d, %d, %d]", r, g, b);
            }

            pointWithCov pv;
            pv.point << pt.x, pt.y, pt.z;
            pv.Semantic_ID = label;
            pv_list_sem.push_back(pv);
        }

        ROS_INFO("Segmentation result converted, total points: %zu", pv_list_sem.size());
        return true;
    }
    else
    {
...
{
    if (latestSegMsg && latestSegMsg->header.seq == msg->header.seq)
    {
        return;
    }
    latestSegMsg = boost::make_shared<sensor_msgs::PointCloud2>(*msg);
}
...
int main(int argc, char **argv)
{
    ...
    nh.param<double>("mapping/max_distance", NUM_MAX_DISTANCE, 80.0);
    nh.param<int>("mapping/update_size_threshold", update_size_threshold, 5);
    nh.param<int>("mapping/init_plane_threshold", init_plane_threshold, 100);
    ...
    ros::Publisher pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2>("/input_cloud", 10);
    ros::Subscriber seg_sub = nh.subscribe("/segmented_cloud", 100, segmentationCallback);
    ...
    while (status)
    {
        ...
        if (sync_packages(Measures))
        {
            ...
            double calc_point_cov_time = std::chrono::duration_cast<std::chrono::duration<double>>(
                                             ...
                                             .count() *
                                         1000;
            std::vector<Eigen::Matrix<double, 6, 1>> delta_history;
            delta_history.reserve(NUM_MAX_ITERATIONS);
            /*** 6. IESEKF Update ***/
            for (iterCount = 0; iterCount < NUM_MAX_ITERATIONS; iterCount++)
            {
                ...
                for (size_t i = 0; i < feats_down_body->size(); i++)
                {
                    ...
                    pv.point_world << world_lidar->points[i].x, world_lidar->points[i].y,
                        world_lidar->points[i].z;
                    double dis_out = feats_down_body->points[i].x * feats_down_body->points[i].x + feats_down_body->points[i].y * feats_down_body->points[i].y + feats_down_body->points[i].z * feats_down_body->points[i].z;
                ...
                else
                {
                    ...
                    Eigen::Vector3d rot_rad = Log(state.rot_end); // rad
                    Eigen::Vector3d trans_m = state.pos_end;      // m
                    Eigen::Vector3d trans_cm = trans_m * 100.0;   // m → cm
                    Eigen::Vector3d rot_deg = rot_rad * 57.3;     // rad → deg

                    Eigen::Matrix<double, 6, 1> state_out;
                    state_out << trans_cm, rot_deg;

                    delta_history.push_back(state_out);
                    int k = int(delta_history.size()) - 1;
                    if ((k >= 2) && (iterCount % 2 != 0))
                    {
                        double tol = 0.001; // cm
                        const Eigen::Matrix<double, 6, 1> &d0 = delta_history[k];
                        const Eigen::Matrix<double, 6, 1> &d2 = delta_history[k - 2];

                        bool converged = true;
                        for (int i = 0; i < 3; ++i)
                        {
                            if (std::abs(d0[i] - d2[i]) >= tol)
                            {
                                converged = false;
                                break;
                            }
            ...
            // publish scan to segment
            if (scanIdx % 50 == 0)
            {
                unique_id_seg++;
                TransformLidar(state, p_imu, surf_feats_undistort, world_lidar_seg);
                publishLidar2seg(pubLaserCloud, world_lidar_seg);
                world_lidar_seg.reset(new pcl::PointCloud<pcl::PointXYZI>());

                std::vector<pointWithCov> pv_list_sem;
                bool sem_test = processSegmentation(pv_list_sem);
                if (sem_test)
                {
                    UpdateSemantic(pv_list_sem, voxel_map);
                }
            }
